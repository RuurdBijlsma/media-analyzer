...
def test_media_analyzer(assets_folder: Path, default_config: AnalyzerSettings, photo_filename: str) -> None:
    ...
    assert isinstance(result.weather_dewpoint, float | None)
    assert isinstance(result.weather_pressure, float | None)
    assert isinstance(result.weather_relative_humidity, float | None)
    assert isinstance(result.weather_recorded_at, datetime | None)
    ...
    assert isinstance(frame_data.is_landscape, bool)
    assert isinstance(frame_data.is_travel, bool)
